...
const getNftInfo = async (req, res) => {
    const query = 'SELECT tokenId, nftName, nftDescription, ticketUri, photoUri, isRegistered FROM nftinfo WHERE ownerAddress = ?; ';
    ...
    try {
        ...
        try {
            ...
            results.isRegistered = rows[0].isRegistered;
        } catch (err) {
            results.result = false;
            results.error.push('Query Error');
        }
